...
/// @dev To use this library, declare `using Revert for bytes4;` and replace `revert CustomError()` with
...
library Revert {
    ...
    /// @notice show up the revert message returned by a call and revert with the selector provided
    /// @dev this function should only be used with custom errors of the type `CustomError(bytes revertReason)`
    function showUpAndRevertWith(bytes4 selector) internal pure {
        assembly ("memory-safe") {
